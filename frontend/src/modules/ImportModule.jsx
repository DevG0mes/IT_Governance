...
import { UploadCloud, FileSpreadsheet, Download, CheckCircle } from 'lucide-react';
        ...
        for (let i = 0; i < previewData.length; i++) {
          ...
          try {
            if (importCategory === 'Colaboradores') {
              ...
              let payload = {};
              if (importCategory === 'Starlinks') { payload = { asset_type: 'Starlink', grupo: getVal(row, 'grupo') || 'N/A', modelo_starlink: getVal(row, 'modelo') || 'N/A', localizacao: getVal(row, 'localizacao', 'local') || 'N/A', responsavel: getVal(row, 'responsavel') || 'N/A', email: getVal(row, 'email_conta', 'email'), senha: getVal(row, 'senha_conta', 'senha'), senha_roteador: getVal(row, 'senha_wifi', 'wifi'), status: 'Disponível' }; }
              else { payload = { asset_type: 'Notebook', patrimonio: safeVal(getVal(row, 'patrimonio')), status: 'Disponível' }; }
              const res = await fetch('http://localhost:8080/api/assets', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(payload) }); const resData = await res.json().catch(() => ({})); if (!res.ok) { throw new Error(resData.error || "Falha ao gravar no banco"); }
            }
            successCount++;
