...
import { Search, X, Trash2, MoreVertical, ListChecks, CheckCircle, PowerOff, Edit2, Printer, Laptop, Smartphone, Cpu, Wifi, Database, Users, ExternalLink } from 'lucide-react';
...
export default function EmployeesModule({ employees, assets, licenses, hasAccess, fetchData, requestConfirm, registerLog }) {
  ...
  const generateTermoPDF = (employee) => {
    ...
    // 👇 FIX: Regra universal para o PDF exibir "Modelo não cadastrado" caso o BD esteja vazio 👇
    const tableData = empAssets.map(a => {
        let descInfo = 'Modelo não cadastrado';
        if (a.asset_type === 'Notebook') {
          descInfo = `${a.notebook?.modelo || 'Modelo não cadastrado'} (Patrimônio: ${a.notebook?.patrimonio || 'S/N'})`;
        } else if (a.asset_type === 'Celular') {
          descInfo = `${a.celular?.modelo || 'Modelo não cadastrado'} ${a.celular?.imei ? `(IMEI: ${a.celular.imei})` : ''}`;
        } else if (a.asset_type === 'CHIP') {
          descInfo = `Linha: ${a.chip?.numero || '-'} (${a.chip?.plano || 'Sem plano'})`;
        } else if (a.asset_type === 'Starlink') {
          descInfo = `${a.starlink?.modelo || 'Modelo não cadastrado'} (Projeto: ${a.starlink?.projeto || '-'})`;
        }
    ...
    addClause("Dos Danos ou Perda", "4 - Em caso de dano ou perda dos equipamentos ou ferramentas por negligência ou mau uso, o(a) RESPONSÁVEL poderá ser responsabilizado(a), conforme previsto no Artigo 462, § 1º da Consolidação das Leis do Trabalho (“§ 1º - Em caso de dano causado pelo empregado, o desconto será lícito, desde que esta possibilidade tenha sido acordada ou na ocorrência de dolo do empregado.”) e nos termos do contrato de prestação de serviços, se aplicável.");
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="col-span-1 border-r border-gray-800 pr-6">
                ...
                <div className="space-y-3">
                  ...
                  {/* 👇 FIX: Mesma regra universal de exibição do PDF para a tela de edição 👇 */}
