...
import type { AccessProfile, SystemUser } from './types/api';

// Durante migração incremental: evitar \"prop mismatch\" nos módulos ainda em JS/JSX.
// Quando cada módulo virar TSX, removemos os casts `as any`.
const DashboardModule = React.lazy(() => import('./modules/DashboardModule')) as any;
const AdminModule = React.lazy(() => import('./modules/AdminModule.tsx')) as any;
const InventoryModule = React.lazy(() => import('./modules/InventoryModule')) as any;
const EmployeesModule = React.lazy(() => import('./modules/EmployeesModule')) as any;
const MaintenanceModule = React.lazy(() => import('./modules/MaintenanceModule')) as any;
const ImportModule = React.lazy(() => import('./modules/ImportModule')) as any;
const ExportModule = React.lazy(() => import('./modules/ExportModule')) as any;
const LicensesModule = React.lazy(() => import('./modules/LicensesModule')) as any;
const ContractsModule = React.lazy(() => import('./modules/ContractsModule')) as any;
const CatalogModule = React.lazy(() => import('./modules/CatalogModule')) as any;
const OffboardingModule = React.lazy(() => import('./modules/OffboardingModule')) as any;
const SettingsModule = React.lazy(() => import('./modules/SettingsModule.tsx')) as any;
...
export default function App() {
  const [currentUser, setCurrentUser] = useState<CurrentUser | null>(() => {
    try {
      ...
      return savedUser && token ? (JSON.parse(savedUser) as CurrentUser) : null;
    } catch {
      return null;
    }
  });
  ...
  const fetchAdminData = useCallback(async () => {
    try {
      ...
      const rawUsers = (usersRes.data?.data || usersRes.data || []) as SystemUser[];
      const rawLogs = logsRes.data?.data || logsRes.data || [];
      const rawProfiles = (profilesRes.data?.data || profilesRes.data || []) as AccessProfile[];

      const mappedProfiles: AccessProfile[] = (Array.isArray(rawProfiles) ? rawProfiles : []).map((p) => ({
        ...p,
        permissions: p.permissions || safeParseJSON(p.permissionsJSON, {}),
      }));

      const profById: Record<number, AccessProfile> = Object.fromEntries(mappedProfiles.map((p) => [p.id, p]));

      const usersFromDB: SystemUser[] = rawUsers.map((u) => {
        let permissions: Record<string, unknown> = {};
        if (u.profile_id && profById[u.profile_id]) {
          permissions = safeParseJSON(profById[u.profile_id].permissionsJSON, {});
        } else {
          const raw = u.permissions_json || u.permissionsJSON;
          permissions = raw ? safeParseJSON(raw, {}) : (roleTemplates[u.cargo || ''] || ({} as PermissionsMap));
        }
        return { ...u, permissions, profileNome: u.profile_id ? profById[u.profile_id]?.nome : null };
      });
  ...
  const registerLog = useCallback(
    async (action: string, module: string, details: string) => {
      ...
      try {
        ...
        if (currentUser?.cargo === 'Administrator' && (activeTab === 'admin' || activeTab === 'settings')) {
  ...
  const fetchData = useCallback(
    (isInitialLoad = false) => {
      ...
      Promise.all(requests)
        .then(() => {
          if (isInitialLoad) Toast.fire({ icon: 'success', title: 'Dados sincronizados com sucesso!' });
        })
        .catch((err: any) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const loggedUser = res.data.data as CurrentUser;

      const rawPerms = loggedUser.permissions_json || loggedUser.permissionsJSON;
      loggedUser.permissions = rawPerms
        ? safeParseJSON(rawPerms, {})
        : (roleTemplates[loggedUser.cargo] || ({} as PermissionsMap));
  ...
  const hasAccess = (module: string, requiredLevel: 'read' | 'edit' = 'read') => {
    if (!currentUser) return false;
    if (currentUser.cargo === 'Administrator') return true;
    const perm = (currentUser.permissions as any)?.[module];
  ...
  const deleteSystemUser = (id: number) => {
    requestConfirm(
      'Excluir Acesso',
      'Tem certeza que deseja excluir este acesso permanentemente?',
      async () => {
        try {
          await api.delete(`/users/${id}`);
          await fetchAdminData();
          await registerLog('DELETE', 'Segurança', `Deletou o usuário ID ${id}`);
        ...
        <div className="w-full flex items-center justify-center p-4">
          <div className="bg-gray-900/80 p-8 rounded-3xl border border-gray-800 w-full max-w-md shadow-2xl backdrop-blur-xl animate-fade-in">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full bg-brandGreen hover:bg-brandGreenHover text-white py-4 rounded-full font-bold shadow-[0_4px_14px_rgba(16,185,129,0.39)] hover:-translate-y-1 transition-all"
              >
                Entrar no Sistema
              </button>
            </form>
          ...
          >
            ...
            <div className="p-4 border-t border-gray-800">
              <button
                ...
                className="w-full flex items-center justify-center p-3 rounded-xl bg-gray-900 text-gray-400 hover:bg-gray-800 hover:text-white transition-colors shadow-sm"
              >
            ...
            <header className="h-16 bg-gray-900/80 backdrop-blur-md border-b border-gray-800 flex items-center justify-between px-6 flex-shrink-0 z-30">
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="bg-gray-800 hover:bg-red-500/20 text-gray-300 hover:text-red-400 p-2.5 rounded-xl transition-colors border border-transparent hover:border-red-500/30"
                >
              ...
              <div
                className={`bg-gray-900 border ${
                  confirmDialog.isDanger
                    ? 'border-red-900/50 shadow-[0_0_30px_rgba(239,68,68,0.2)]'
                ...
                <div className="flex justify-between items-center mb-4">
                  <h2
                    className={`text-xl font-bold flex items-center gap-2 ${
                      confirmDialog.isDanger ? 'text-red-500' : 'text-white'
                    }`}
                  >
                    {confirmDialog.isDanger ? (
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl font-bold ${
                      confirmDialog.isDanger ? 'bg-red-600 text-white' : 'bg-brandGreen text-white'
                    }`}
                  >
